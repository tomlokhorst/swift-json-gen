...
function processCmdArgs(cb) {
    ...
    if (typeof (outputDirectory) == 'string') {
        mkdirp(outputDirectory, function (err) { return cb(err, inputs, stathamDirectory, outputDirectory); });
    }
    else {
...
function handleFiles(inputs, stathamDirectory, outputDirectory) {
    var filenames = inputs.flatMap(fullFilenames);
    if (stathamDirectory) {
        console.log('statham ', stathamDirectory);
