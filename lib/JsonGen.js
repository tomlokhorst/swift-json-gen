...
function generate() {
    ...
    exec(swiftc + ' --version', function (error, stdout, stderr) {
        ...
        processCmdArgs(function (error, inputs, accessLevel, stathamDir, outputDirectory) {
            return handleFiles(inputs, accessLevel, stathamDir, outputDirectory);
        });
    });
}
function processCmdArgs(cb) {
    ...
    var accessLevel = typeof (argv.accessLevel) == 'string' ? argv.accessLevel : null;
    ...
    function afterStatham(stathamTempDir) {
        if (typeof (outputDirectory) == 'string') {
            mkdirp(outputDirectory, function (err) { return cb(err, inputs, accessLevel, stathamTempDir, outputDirectory); });
        }
        else {
