...
//
function decoders(ast) {
    var emptyAliases = {};
    var ds1 = ast
        .children('struct_decl')
        .map(function (s) { return hasDecoder(s) ? struct(s, emptyAliases)[0].baseName : null; });
    var ds2 = ast
        .children('extension_decl')
        .map(function (s) { return hasDecoder(s) ? extension(s).typeBaseName : null; });
    var ds = ds1.concat(ds2)
        .filter(function (n) { return n != null; })
        .map(function (d) { return { typeBaseName: d }; });
    return ds;
}
exports.decoders = decoders;
function hasDecoder(ast) {
    var xs = ast.children('func_decl').filter(function (f) { return f.key(0) == 'decode'; });
    return xs.length > 0;
}
...
function structs(ast, aliases) {
    return ast.children('struct_decl').flatMap(function (a) { return struct(a, aliases); });
}
...
function struct(ast, aliases, prefix) {
    ...
    var varDecls = ast.children('var_decl').map(function (a) { return varDecl(a, aliases); });
    var r = { baseName: baseName, typeArguments: typeArgs, varDecls: varDecls };
    var rs = ast.children('struct_decl').flatMap(function (a) { return struct(a, aliases, baseName); });
    return [r].concat(rs);
}
