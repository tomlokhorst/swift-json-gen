...
function funcNames(ast) {
    return ast.children('func_decl').map(function (f) { return f.key(0) == 'implicit' ? f.key(1) : f.key(0); });
}
function constructorParams(ast) {
    return ast.children('constructor_decl')
        .filter(function (a) { return a.attr('access') != 'private' && a.attr('access') != 'fileprivate'; })
        .flatMap(constructorDeclParams);
}
function constructorDeclParams(constructorDecl) {
    var parts = constructorDecl
        .filter(function (obj) { return (typeof (obj) == 'object' && obj.length == 0) || (typeof (obj) == 'object' && obj.length == 1 && typeof (obj[0]) == 'string'); })
        .map(function (a) { return a.length ? a[0] : ''; });
    if (parts.length < 3) {
        return [];
    }
    var names = parts[0];
...
function structs(ast, aliases, prefix) {
    var structs1 = ast.children('struct_decl')
        .map(function (a) { return struct(a, aliases, prefix); });
    var structs2 = ast.children(['struct_decl', 'enum_decl'])
        .flatMap(function (a) { return structs(a, aliases, getBaseName(a, prefix)); });
...
function struct(ast, aliases, prefix) {
    ...
    var varDecls = ast.children('var_decl')
        .filter(function (a) { return a.attr('storage_kind') == 'stored' || a.attr('storage_kind') == 'stored_with_trivial_accessors'; })
...
function enums(ast, aliases, prefix) {
    var enums1 = ast
        ...
        .map(function (a) { return enum_(a, aliases, prefix); });
    var enums2 = ast.children(['struct_decl', 'enum_decl'])
        .flatMap(function (a) { return enums(a, aliases, getBaseName(a, prefix)); });
