...
Array.prototype.fields = function () {
    var fields = this.slice(1).filter(not(isArray));
    return fields;
};
Array.prototype.keys = function () {
    return this.fields().filter(not(isAttr)).map(function (s) {
        var val = s.replace(/'/g, '"');
        if (val.length && val[0] == '"')
            val = JSON.parse(val);
        return val;
...
Array.prototype.attrs = function () {
    return this.fields().filter(isAttr).map(function (s) {
        var ix = s.indexOf('=');
...
Array.prototype.attr = function (key) {
    var attrs = this.attrs().filter(function (arr) {
        return arr[0] == key;
    });
    if (!attrs.length)
...
function parse(text, multiple) {
    var results = [];
    text = text.trim();
    text = text.replace(/='([^']*)'/g, function (n) {
        return n.replace(/ /g, 'JSON_GEN_SPACE');
    });
    if (text.charAt(0) != '(')
        return text;
    var stack = [];
