...
function structs(ast) {
    return ast.children('struct_decl').flatMap(function (a) { return struct(a); });
}
exports.structs = structs;
function struct(ast, prefix) {
    prefix = prefix ? prefix + '.' : '';
    // strip generic type
    var fullName = ast.key(0);
    var baseName = prefix + fullName.replace(/<([^>]*)>/g, '');
    var r = { baseName: baseName, data: ast };
    var rs = ast.children('struct_decl').flatMap(function (a) { return struct(a, baseName); });
    return [r].concat(rs);
}
