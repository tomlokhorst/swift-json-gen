...
function makeFile(file, accessLevel, globalAttrs, filename) {
    var accessLevelPrefix = accessLevel == null ? '' : accessLevel + ' ';
    function constructorExists(struct) {
        var paramNames = struct.varDecls.map(function (vd) { return vd.name + ':'; });
        var paramTypes = struct.varDecls.map(function (vd) { return typeString(vd.type); });
        var paramsString = paramNames.join('') + '||' + paramTypes.join(', ');
...
function decodeArguments(struct) {
    var parts = struct.typeArguments
        .map(function (typeVar) { return '_ decode' + typeVar + ': @escaping (Any) throws -> ' + escaped(typeVar); });
    return '(' + parts.join(', ') + ')';
}
...
function encodeArguments(struct) {
    var parts = struct.typeArguments
        .map(function (typeVar) { return '_ encode' + typeVar + ': (' + escaped(typeVar) + ') -> Any'; });
    return '(' + parts.join(', ') + ')';
}
