...
function makeFile(file, aliases, decoders, filename) {
    var lines = [];
    lines.push('//');
    lines.push('//  ' + filename);
    lines.push('//');
    ...
    function decoderExists(struct) {
        return decoders.map(function (d) { return d.typeBaseName; }).contains(struct.baseName);
    }
    var structs = ast.structs(file, aliases).filter(function (s) { return !decoderExists(s); });
